...
void preencheBancoPacientesGerenciador(Gerenciador *ger){
    ...
    int numPacientes;
    scanf("%d%*c", &numPacientes); // Lê o número de pacientes
    ...
    for (int i = 0; i < numPacientes; i++) {
        ...
        int numLesoes;
        scanf("%d%*c", &numLesoes); // Lê o número de lesões
        
        // Lê e adiciona cada lesão
        ...
        // Adiciona o paciente ao banco
        if (ger->tamBanco >= ger->tamBancoAlocado) {
            size_t novoTamanho = ger->tamBancoAlocado * 2;
            Paciente **temp = (Paciente**)realloc(ger->bancoPacientes, 
                                                novoTamanho * sizeof(Paciente*));
            if (!temp) {
                liberaPaciente(p);
                continue;
            }
            ger->bancoPacientes = temp;
            ger->tamBancoAlocado = novoTamanho;
        }
        
        ger->bancoPacientes[ger->tamBanco] = p;
        ger->tamBanco++;
    }
}
